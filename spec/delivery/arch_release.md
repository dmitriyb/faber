...
Both signatures are verified before either binary is touched, and on any mid-swap failure both roll back from their `.bak` — so the pair is never left mismatched (either both old or both new), which the config-side active-runs guard ("faber is not upgraded mid-run") makes safe by guaranteeing no live run spans the brief window between the two renames.
Upgrade mode is **forward-only**: on the latest path (`VERSION` unset) it resolves GitHub's latest release and orders it against `--current` — equal → already up to date, exit 0; newer → proceed; older → HARD-REFUSE and exit non-zero, non-overridable, because a "latest" that moved backward is a rollback anomaly (a compromised origin serving an old but validly-signed release as latest). A `--check`/`--dry-run` on that older-latest reports the anomaly as a warning but still exits 0 (its job is to report, not to gate); a `dev`/unstamped `--current` cannot be ordered, so the latest path warns and proceeds rather than blocks.
On the explicit-version path (`VERSION` set — the `faber upgrade --version vX.Y.Z` case) the operator named the release, so there is no untrusted latest to attack: the script installs it in any direction with no anomaly refusal, printing an "older than installed, as explicitly requested" notice when applicable.
It also carries `--rollback` (restore both from `.bak`), a `RequireRoot` check with a clear "re-run elevated" message (no auto-sudo, unlike install mode's `sudo` retry), and a `--dry-run` that resolves and verifies without touching disk.
The exact target paths and the mode signals arrive as flags (`--upgrade`/`--rollback`/`--check`, `--target`/`--box-target`, `--current`) from the `faber upgrade` command — a self-documenting operator-facing contract; only the release pin (`VERSION`) and the test-only origin bases stay env.
There is no `--force` flag in the script: the active-runs guard is resolved entirely by the `faber upgrade` command (Go-side), and the forward-only anomaly refusal is deliberately not overridable, so the script exposes no override for either.
