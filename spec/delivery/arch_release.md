...
The same `install.sh` carries an upgrade mode (`FABER_UPGRADE=1`) reusing its identical resolve/download/SSHSIG-verify path, but replacing the two currently-installed binaries in place instead of installing into `INSTALL_DIR`.
...
The exact target paths and the mode signals arrive as environment (`FABER_TARGET`/`FABER_BOX_TARGET`, `VERSION`, `FABER_CURRENT_VERSION`, `FABER_UPGRADE_FORCE`/`FABER_ROLLBACK`/`FABER_DRY_RUN`) from the `faber upgrade` command.
